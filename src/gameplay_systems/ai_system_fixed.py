...
class AISystem:
    ...
    def __init__(self, game_state_or_manager):
        ...
        # Configure logging - use module name for better integration with app.py logging
    ...
    def select_strategic_goal(self, unit) -> Optional[Goal]:
        ...
        # Use the strategic evaluator to select the best goal
        # This will generate detailed logs about the goal selection process
        # Pass the game_state_manager instead of game_state to ensure get_units_by_faction is available
        game_state_for_strategic = self.game_state_manager if self.game_state_manager else self.game_state
        return self.strategic_evaluator.select_best_goal(
            unit,
            game_state_for_strategic,
            persona
        )