...
- Stat penalties: Str, Mag, Skl, Spd, and Def are all halved (rounded down) when rescuing
- Movement penalty: Movement is halved (rounded down) if Carried Con > Carrier Con / 2
...
def apply_rescue_penalties(rescuer: Unit) -> dict:
    """
    Apply standard rescue penalties to a unit.
    
    Args:
        rescuer: The rescuing unit
        
    Returns:
        Dictionary with the penalized stats
    """
    penalized_stats = rescuer.stats.copy()
    
    # Halve all combat stats
    for stat in ['str', 'mag', 'skl', 'spd', 'def']:
        penalized_stats[stat] = penalized_stats[stat] // 2
    
    return penalized_stats
...
def print_rescue_scenario(rescuer: Unit, target: Unit):
    ...
    if can_rescue_result:
        penalized_stats = apply_rescue_penalties(rescuer)
        
        print(f"\nOriginal stats: {rescuer.stats}")
        print(f"Penalized stats: {penalized_stats}")
        
        print(f"\nMovement penalty: {movement_penalty}")
        print(f"Threshold for movement penalty: {rescuer.con / 2:.1f} (half of actual Con)")
        
        original_mov = rescuer.stats.get('mov', 0)
        new_mov = original_mov
        if movement_penalty:
            new_mov = original_mov // 2
        print(f"Original MOV: {original_mov}, New MOV: {new_mov}")
